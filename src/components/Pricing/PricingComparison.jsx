...
function ComparisonCell({ value }) {
  ...
  if (value === false) {
    return <span className="text-muted-foreground dark:text-foreground">-</span>;
  }
  ...
  return (
    <section id="compare-plans" className="py-10 sm:py-14">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="overflow-x-auto overscroll-x-contain rounded-2xl border border-border bg-card/90 shadow-[0_20px_42px_-28px_rgba(15,23,42,0.5)] dark:border-border dark:bg-background/85">
          <table className="w-full min-w-170 text-xs sm:min-w-190 sm:text-sm">
            <thead>
              <tr className="text-left text-foreground">
                <th className="sticky left-0 z-20 min-w-45 border-b border-border bg-card px-3 py-3 font-semibold sm:min-w-55 sm:px-4 dark:border-border dark:bg-background">
                  Feature
                </th>
              ...
              {categories.map((category) => (
                <Fragment key={category.id}>
                  ...
                  {category.rows.map((row) => (
                    ...
                    >
                      <td className="sticky left-0 z-10 bg-card px-3 py-3 font-medium sm:px-4 dark:bg-background">{row.feature}</td>
