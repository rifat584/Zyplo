  ...
  return (
    <aside className="rounded-xl border border-slate-200 bg-white/80 p-4 dark:border-white/10 dark:bg-slate-900/70">
      <div className="mb-3 flex items-center justify-between">
        ...
        <Link href="/dashBoardTest/my-work" className="text-xs text-indigo-600 hover:text-indigo-700 dark:text-indigo-300">
          Open
        </Link>
      </div>
      ...
      <div className="space-y-2">
        {myWorkPreview.items.length ? (
          myWorkPreview.items.map((task) => (
            <Link
              key={task.id}
              href={`/dashBoardTest/w/${task.project?.workspaceId}/p/${task.projectId}/list`}
