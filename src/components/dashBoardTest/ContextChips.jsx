...
export default function ContextChips() {
  ...
  const onSwitchWorkspace = (nextWorkspaceId) => {
    ...
    if (pathname.includes("/p/") && nextProjects.length > 0) {
      router.push(`/dashBoardTest/w/${nextWorkspaceId}/p/${nextProjects[0].id}`);
      return;
    }

    router.push(`/dashBoardTest/w/${nextWorkspaceId}`);
  };
  ...
  const onSwitchProject = (nextProjectId) => {
    ...
    if (pathname.endsWith("/list")) {
      router.push(`/dashBoardTest/w/${activeWorkspace.id}/p/${nextProjectId}/list`);
      return;
    }
    if (pathname.endsWith("/activity")) {
      router.push(`/dashBoardTest/w/${activeWorkspace.id}/p/${nextProjectId}/activity`);
      return;
    }
    router.push(`/dashBoardTest/w/${activeWorkspace.id}/p/${nextProjectId}`);
  };
  ...
  return (
    <div className="min-w-0">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {pathname === "/dashBoardTest/workspaces" ? (
          <>
          ...
          <>
            <span className="text-lg font-semibold text-slate-900 dark:text-slate-100">
              {pathname === "/dashBoardTest/my-work" ? "My Work" : "Activity"}
            </span>
          </>
        ) : (
          <>
