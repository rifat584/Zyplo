...
export default function AppSidebar({ mobileOpen, onCloseMobile }) {
  ...
  const isGlobalPage = pathname === "/dashBoardTest/my-work" || pathname === "/dashBoardTest/activity";
  ...
  const settingsHref = activeWorkspaceId
    ? projectId
      ? `/dashBoardTest/w/${activeWorkspaceId}/p/${projectId}/settings`
      : `/dashBoardTest/w/${activeWorkspaceId}/settings`
    : "/dashBoardTest/workspaces";
  const membersHref = activeWorkspaceId ? `/dashBoardTest/w/${activeWorkspaceId}/members` : "/dashBoardTest/workspaces";

  const quickLinks = useMemo(() => {
    const activityHref = workspaceId ? `/dashBoardTest/w/${workspaceId}/activity` : "/dashBoardTest/activity";
    return [
      { href: "/dashBoardTest/my-work", label: "My Work", icon: CheckSquare },
  ...
  const content = (
    <>
      ...
      {!isGlobalPage && pathname !== "/dashBoardTest/workspaces" ? (
      ...
      <div className={`space-y-1 py-4 ${effectiveCollapsed ? "flex flex-col items-center" : ""}`}>
        <NavItem href="/dashBoardTest/workspaces" label="All Workspaces" icon={Home} active={pathname === "/dashBoardTest/workspaces"} collapsed={effectiveCollapsed} onClick={onCloseMobile} />
      </div>
